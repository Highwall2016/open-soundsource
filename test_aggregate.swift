...
if let processObjectID = processObjectID {
    ...
    // Create an aggregate device using the tap as input and defaultOutput as output!
    var subDevices = [tapID, defaultOutput]
    // Note: CoreAudio expects an array of device UIDs (String), NOT AudioDeviceIDs for the SubDeviceList!
    ...
    var outUID: CFString? = nil
    AudioObjectGetPropertyData(defaultOutput, &uidAddr, 0, nil, &uidSize, &outUID)
    
    print("Tap UID: \(String(describing: tapUID)), Output UID: \(String(describing: outUID))")
    
    if let tapUID = tapUID as? String, let outUID = outUID as? String {
        ...
        if err == noErr {
            ...
            AudioUnitSetProperty(engine.outputNode.audioUnit!, kAudioOutputUnitProperty_EnableIO, kAudioUnitScope_Output, 1, &enableIO, UInt32(MemoryLayout<UInt32>.size))
